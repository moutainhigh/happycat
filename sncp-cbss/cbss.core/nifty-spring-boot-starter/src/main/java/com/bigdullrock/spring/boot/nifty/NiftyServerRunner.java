...
public class NiftyServerRunner implements ApplicationRunner, DisposableBean {
	...
	public void run(final ApplicationArguments args)
		...
		for (Map.Entry<String, Object> niftyHandler : applicationContext.getBeansWithAnnotation(NiftyHandler.class).entrySet()) {
			List<Class<?>> handlerInterfaces = findAllInterfaces(niftyHandler.getValue().getClass());

			Map<String, Class<?>> classes = getInterfaceAndProcessor(handlerInterfaces);
			...
			LOG.info("Registering Nifty processor {}: {}", (processorName + "." + niftyHandler.getKey()).toLowerCase(), tproc);
			multiplexedProcessor.registerProcessor((processorName + "." + niftyHandler.getKey()).toLowerCase(), tproc);
		}

		ThriftServerDefBuilder thriftServerDefBuilder = new ThriftServerDefBuilder().withProcessor(multiplexedProcessor);
		...
		if (niftyServerProperties != null) {
			...
			if (niftyServerProperties.getClientIdleTimeout() != null) {
				thriftServerDefBuilder.clientIdleTimeout(new Duration(niftyServerProperties.getClientIdleTimeout().doubleValue(), TimeUnit.MILLISECONDS));
			}
			if (niftyServerProperties.getMaxConnections() != null) {
				thriftServerDefBuilder.limitConnectionsTo(niftyServerProperties.getMaxConnections());
			}
			if (niftyServerProperties.getTaskTimeout() != null) {
				thriftServerDefBuilder.taskTimeout(new Duration(niftyServerProperties.getTaskTimeout(), TimeUnit.MILLISECONDS));
			}
			if (niftyServerProperties.getQueueTimeout() != null) {
				thriftServerDefBuilder.queueTimeout(new Duration(niftyServerProperties.getQueueTimeout(), TimeUnit.MILLISECONDS));
			}
			if (niftyServerProperties.getQueuedResponseLimit() != null) {
				thriftServerDefBuilder.limitQueuedResponsesPerConnection(niftyServerProperties.getQueuedResponseLimit());
			}
		}
	...
	private Map<String, Class<?>> getInterfaceAndProcessor(final List<Class<?>> handlerInterfaces)
		...
		for (Class<?> handlerInterfaceClass : handlerInterfaces) {
			if (!handlerInterfaceClass.getName().endsWith("$Iface") || handlerInterfaceClass.getDeclaringClass() == null) {
				continue;
			}
			for (Class<?> innerClass : handlerInterfaceClass.getDeclaringClass().getDeclaredClasses()) {
				if (!innerClass.getName().endsWith("$Processor") || !TProcessor.class.isAssignableFrom(innerClass)) {
					continue;
				}
				if (ifaceClass != null) {
	...
	/**
	 * proxyTargetClass = true might be set, so we have to work harder to get
	 * the Thrift Iface.
	 */
	private List<Class<?>> findAllInterfaces(Class<?> handlerClass)
