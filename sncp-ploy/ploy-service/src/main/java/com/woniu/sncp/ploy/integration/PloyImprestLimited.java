...
import com.woniu.sncp.core.errorcode.ErrorCode;
...
public class PloyImprestLimited {
	...
	public Message<PloyTypeStatDTO>  returnMaxPointLimited(Message<PloyTypeStatDTO> input) {
		String returnMaxPoint = StringUtils.substringBetween(input.getPayload().getPresentsPloy().getLimitContent() + "|",
				 "ReturnMaxPoint:", "|");
		...
		if (StringUtils.isNotBlank(returnMaxPoint)) {
			...
			//查询用户返点记录
			Long returnAmount = imprestService.findSumLargessPoints(passportDto.getId(), start, end, input.getPayload().getPresentsPloy().getType(), null);
			if(returnAmount == null || returnAmount.longValue() >= Long.valueOf(returnMaxPoint).longValue()) {
				logger.debug("充值累计返点活动 最高返回，不再赠送.....");
				logger.debug("passportId: " + passportDto.getId() + ",已返点记录: " + returnAmount + ",本次返点最大值: " + returnMaxPoint);
				input.getPayload().clean();
			}
		}
		return input;
	...
	public Message<PloyTypeStatDTO> imprestAmountLimited(Message<PloyTypeStatDTO> input) {
		String imprestAmount = StringUtils.substringBetween(input.getPayload().getPresentsPloy().getLimitContent() + "|",
				 "imprestAmount:", "|");	
		if (StringUtils.isNotBlank(imprestAmount)) {
			...
			Date start = DateUtils.truncate(imprestLog.getImprestDate(), Calendar.DATE);
			Date end = DateUtils.truncate( DateUtils.addDays(imprestLog.getImprestDate(), 1), Calendar.DATE);
            Long _imprestAmount = imprestService.findSumImprestAmount(Long.valueOf(ployRequestDTO.getGameId()), passportDto.getId(), start, end);
